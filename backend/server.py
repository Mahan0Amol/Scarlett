...
@app.get("/status")
async def status():
    return {"status": "running", "service": "Scarlett Backend"}
...
async def start_audio(sid, data=None):
    ...
    if audio_loop:
        if loop_task and (loop_task.done() or loop_task.cancelled()):
             ...
             await sio.emit('status', {'msg': 'Scarlett Already Running'})
             return
    ...
    try:
        ...
        print("Emitting 'Scarlett Started'")
        await sio.emit('status', {'msg': 'Scarlett Started'})
        ...
        print(f"CRITICAL ERROR STARTING Scarlett: {e}")
        import traceback
        traceback.print_exc()
...
async def stop_audio(sid):
    ...
    if audio_loop:
        ...
        audio_loop = None
        await sio.emit('status', {'msg': 'Scarlett Stopped'})
...
async def discover_kasa(sid):
    ...
    try:
        ...
        save_settings()
        print(f"[SERVER] Saved {len(saved_devices)} light devices to settings.")

    except Exception as e:
        print(f"Error discovering lights: {e}")
        await sio.emit('error', {'msg': f"Light Discovery Failed: {str(e)}"})
...
async def discover_door(sid):
    ...
    try:
        ...
        SETTINGS["door_devices"] = saved_devices
        save_settings()
        print(f"[SERVER] Saved {len(saved_devices)} door devices to settings.")

    except Exception as e:
        print(f"Error discovering lights: {e}")
        await sio.emit('error', {'msg': f"Door Discovery Failed: {str(e)}"})
...
async def discover_printers(sid):
    ...
    if not audio_loop or not audio_loop.printer_agent:
        ...
        if saved_printers:
            ...
            await sio.emit('status', {'msg': "Connect to Scarlett to enable printer discovery"})
            return
...
async def control_kasa(sid, data):
    ...
    print(f"Light Control: {ip} -> {action}")

    try:
        success = False

        if action == "on":
            r = requests.post(f"http://{ip}/led/on", timeout=5)
            success = r.status_code == 200

        elif action == "off":
            r = requests.post(f"http://{ip}/led/off", timeout=5)
            success = r.status_code == 200

        elif action == "brightness":
            val = data.get('value')
            r = requests.post(f"http://{ip}/command", data=f"brightness:{val}", timeout=5)
            success = r.status_code == 200

        elif action == "color":
            h = data.get('value', {}).get('h', 0)
            r = requests.post(f"http://{ip}/command", data=f"color:{h}", timeout=5)
            success = r.status_code == 200
        ...
        await sio.emit('error', {'msg': f"Light Control Error: {str(e)}"})
@sio.event
async def control_door(sid, data):
    ip = data.get('ip')
    action = data.get('action')
    print(f"Door Control: {ip} -> {action}")

    try:
        success = False

        if action == "on":
            r = requests.post(f"http://{ip}/relay/on", timeout=5)
            success = r.status_code == 200

        elif action == "off":
            r = requests.post(f"http://{ip}/relay/off", timeout=5)
            success = r.status_code == 200

        if success:
            await sio.emit('door_update', {
                'ip': ip,
                'is_on': True if action == "on" else (False if action == "off" else None),
            })
        else:
...
@sio.event
async def execute_cmd(sid, data):
    command = data.get('command', '')
    print(f"[CMD] Executing: {command}")
    
    try:
        result = await cmd_agent.execute_command(command)
        
        if result.get('clear'):
            await sio.emit('cmd_clear', room=sid)
        elif 'error' in result:
            await sio.emit('cmd_error', {'error': result['error']}, room=sid)
        else:
            await sio.emit('cmd_output', {
                'output': result['output'],
                'current_dir': result.get('current_dir'),
                'from_ai': result.get('from_ai', False)
            }, room=sid)
            
    except Exception as e:
        print(f"[CMD] Error: {e}")
        await sio.emit('cmd_error', {'error': str(e)}, room=sid)

@sio.event
async def get_cmd_status(sid):
    display_dir = cmd_agent.current_dir.replace(os.path.expanduser('~'), '~')
    await sio.emit('cmd_output', {
        'output': 'Terminal ready. Type "help" for available commands.',
        'current_dir': display_dir
    }, room=sid)

@sio.event
async def autocomplete_cmd(sid, data):
    partial = data.get('partial', '')
    matches = cmd_agent.get_autocomplete(partial)
