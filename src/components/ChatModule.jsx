    ...
    return (
        ...
        `}
            style={{
                left: position.x,
                top: position.y,
                transform: 'translate(65%, -56%)', // Aligned top-center
                width: 400,
                height: height
            }}
        >
            ...
            <div
                ...
                style={{ height: height ? `calc(${height}px - 90px)` : '15rem' }}
            >
                {messages.slice(-15).map((msg, i) => (
                    <div key={i} className="text-sm border-l-2 border-red-800/50 pl-3 py-1">
                        <span className="text-red-600 font-mono text-xs opacity-70">[{msg.time}]</span> <span className="font-bold text-red-300 drop-shadow-sm">{msg.sender}</span>
            ...
            <div className="flex gap-2 relative z-10 absolute bottom-4 left-6 right-6">
                <input
                    ...
                    className="flex-1 bg-black/40 border border-red-700/30 rounded-lg p-3 text-red-50 focus:outline-none focus:border-red-400 focus:ring-1 focus:ring-red-400/50 transition-all placeholder-red-800/50 backdrop-blur-sm"
                    style={{
