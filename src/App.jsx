...
function App() {
    ...
    const [doorDevices, setDoorDevices] = useState([]);
    ...
    useEffect(() => {
        ...
        if (isConnected && isAuthenticated && socketConnected && micDevices.length > 0 && !hasAutoConnectedRef.current) {
            ...
            // Trigger Kasa and Printer and Door Discovery
    ...
    useEffect(() => {
        ...
        socket.on('status', (data) => {
            ...
            if (data.msg === 'Scarlett Started') {
                setStatus('Model Connected');
            } else if (data.msg === 'Scarlett Stopped') {
                setStatus('Connected');
            }
        });
        ...
        // Auto-confirm tool requests (no popup)
        socket.on('tool_confirmation_request', (data) => {
            console.log("Auto-confirming tool request:", data);
        ...
        // Door Devices
        socket.on('door_devices', (devices) => {
            console.log("Door Devices:", devices);
            setDoorDevices(devices);
        });

        socket.on('door_update', (data) => {
            setDoorDevices(prev => prev.map(d => {
                if (d.ip === data.ip) {
                    // Update only fields that are not null/undefined
                    return {
                        ...d,
            ...
            if (results.landmarks && results.landmarks.length > 0) {
                ...
                if (isFist) {
                    if (!activeDragElementRef.current) {
                        ...
                        const draggableElements = ['cad', 'browser', 'kasa', 'door', 'printer'];
    ...
    const toggleDoorWindow = () => {
        setShowDoorWindow(!showDoorWindow);
    };
    ...
    return (
        <div className="h-screen w-screen bg-black text-red-100 font-mono overflow-hidden flex flex-col relative selection:bg-red-900 selection:text-white">
            ...
            {isVideoOn && isHandTrackingEnabled && (
                <div
                    className={`fixed w-6 h-6 border-2 rounded-full pointer-events-none z-[100] transition-transform duration-75 ${isPinching ? 'bg-red-400 border-red-400 scale-75 shadow-[0_0_15px_rgba(34,211,238,0.8)]' : 'border-red-400 shadow-[0_0_10px_rgba(34,211,238,0.3)]'}`}
                    style={{
                        left: cursorPos.x,
                        top: cursorPos.y,
            ...
            <div
                className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] bg-red-900/10 rounded-full blur-[120px] pointer-events-none"
            />
            ...
            <div className="z-50 flex items-center justify-between p-2 border-b border-red-500/20 bg-black/40 backdrop-blur-md select-none sticky top-0" style={{ WebkitAppRegion: 'drag' }}>
                <div className="flex items-center gap-4 pl-2">
                    <h1 className="text-xl font-bold tracking-[0.2em] text-red-400 drop-shadow-[0_0_10px_rgba(34,211,238,0.5)]">
                        Scarlett
                    </h1>
                    <div className="text-[10px] text-red-700 border border-red-900 px-1 rounded">
                        V2.0.0
                    </div>
                    {/* FPS Counter */}
                    {isVideoOn && (
                ...
                <div className="flex items-center gap-2 pr-2" style={{ WebkitAppRegion: 'no-drag' }}>
                    ...
                    <button onClick={handleMinimize} className="p-1 hover:bg-red-900/50 rounded text-red-500 transition-colors">
                        <Minus size={18} />
                    </button>
                    <button onClick={handleMaximize} className="p-1 hover:bg-red-900/50 rounded text-red-500 transition-colors">
            ...
            />
                ...
                <div
                    ...
                    `}
                    style={{ zIndex: 20, transform: 'translate(-180%, -45%)' }}
                >
                    <div className="absolute inset-0 bg-[url('https://grainy-gradients.vercel.app/noise.svg')] opacity-5 pointer-events-none mix-blend-overlay"></div>
                    {/* Compact Display Container (1080p Source) */}
                    <div className="relative border border-red-500/30 rounded-lg overflow-hidden shadow-[0_0_20px_rgba(6,182,212,0.1)] w-80 aspect-video bg-black/80">
                        ...
                        <div className="absolute top-2 left-2 text-[10px] text-red-400 bg-black/60 backdrop-blur px-2 py-0.5 rounded border border-red-500/20 z-10 font-bold tracking-wider">CAM_01</div>
                ...
                {showCadWindow && (
                    ...
                    >
                        ...
                        >
                            <span className="text-xs font-bold tracking-widest text-red-500/70">CAD PROTOTYPE</span>
                            <button
                ...
                <ChatModule
                    ...
                    height={elementSizes.chat.h + 500}
                ...
                <div className="z-20 flex justify-center pb-10 pointer-events-none">
                    <ToolsModule
                        ...
                        onToggleDoor={toggleDoorWindow}
                        showKasaWindow={showKasaWindow}
                        showDoorWindow={showDoorWindow}
                ...
                {showDoorWindow && (
                    <DoorWindow
                        socket={socket}
                        position={elementPositions.door}
                        activeDragElement={activeDragElement}
                        setActiveDragElement={setActiveDragElement}
                        devices={doorDevices}
                        onClose={() => setShowDoorWindow(false)}
                        onMouseDown={(e) => handleMouseDown(e, 'door')}
                        zIndex={getZIndex('door')}
                    />
                )}
                ...
                {/* CMD Window - Left Side - FIXED */}
                <div
                    id="cmd"
                    className="absolute flex flex-col backdrop-blur-xl bg-black/40 border border-gray-500/20 shadow-2xl overflow-hidden rounded-xl"
                    style={{
                        left: '280px',
                        top: '50%',
                        transform: 'translate(-28%, -50%)',
                        width: `${elementSizes.cmd?.w - 200}px`,    // عرض ثابت
                        height: `${elementSizes.cmd?.h + 528}px`,   // ارتفاع ثابت
                        pointerEvents: 'auto',
                ...
                >
                    ...
                    <div className="relative z-20 flex-1 min-h-0 overflow-hidden">
                        <CmdWindow 
                            socket={socket} 
                            onClose={() => setShowCmdWindow(false)} 
                        />
                    </div>
                </div>
